...
const workflowSteps = [
  ...
  '保存ボタンからJSONスナップショット／統合CSVをダウンロードし、必要に応じて自動復元を有効化します。',
  'PDFレポート出力で共有用資料を生成し、必要なセクションやカラーモードを選択します。',
  '必要に応じてCSVやJSONを差し替え、データステータスの警告／エラーを解消してからダッシュボードを共有します。'
];
...
const cautions = [
  ...
  '保存したJSONはローカル環境にのみ保持されます。共有する場合はアクセス権限を確認してください。',
  'エラーが発生したCSVやJSONはダッシュボードに反映されないため、ステータスで内容を確認の上、再読み込みしてください。',
  '集計は選択月に応じて自動更新されます。データ期間をまたぐ分析を行う場合は「全期間」を選択してください。',
  'GitHub Pagesへの公開はGitHub Actionsが担当します。`dist`はコミット不要です。'
];

...
売上分析(`/sales`)は売上CSVをローカル保存し、月別サマリ・曜日平均・日別推移をカードとグラフで表示します。ファイル名から年月を自動推定し、アップロード済みの月を切り替えて詳細テーブルをトグル展開できます。[src/app/sales/page.tsx:1-400][src/lib/salesData.ts:1-260][src/components/sales/MonthlySalesChart.tsx:1-120][src/components/sales/WeekdaySalesAverageChart.tsx:1-120][src/components/sales/DailySalesChart.tsx:1-140]

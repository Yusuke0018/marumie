...
ローカルストレージへの読込・保存は予約・リスティング・アンケート・売上・診断・経費などの型付きヘルパーを経由し、生データへ直接アクセスしません。[src/lib/reservationData.ts:1-320][src/lib/listingData.ts:1-200][src/lib/surveyData.ts:1-220][src/lib/salesData.ts:1-280][src/lib/diagnosisData.ts:1-520][src/lib/expenseData.ts:1-200]
データ共有時は `SharedDataBundle` 型を用いて複数データソースをまとめ、バージョンと生成日時を含めて送受信します。[src/lib/sharedBundle.ts:1-24]
...
- 売上CSVは患者分析ページのデータ管理経由で取り込み、`salesData` ヘルパーを介して保存・共有する。[src/app/patients/page.tsx:5908-6390][src/lib/salesData.ts:1-280]
